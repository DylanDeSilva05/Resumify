...
class CVAnalyzer:
    ...
    def _analyze_education_match(
        ...
        # If no education requirements specified, give neutral/good score if candidate has education
        if not required_education:
            # Check if candidate has any education
            if candidate_education and len(candidate_education) > 0:
                return {'score': 75.0, 'matched': [], 'details': {'no_requirements': True}}
            else:
                return {'score': 50.0, 'matched': [], 'details': {'no_requirements': True}}
        ...
        score = (len(matched) / len(required_education)) * 100 if required_education else 75.0
    ...
    def _analyze_experience_match(
        ...
        if min_required == 0:
            # No experience requirement specified, give good score if they have experience
            if candidate_years > 0:
                score = 75.0  # Has experience, no specific requirement
            else:
                score = 50.0  # No experience, no requirement
